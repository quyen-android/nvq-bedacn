...
class ChuyenDi(Base):
    ...
    nguoi_dung = relationship(
        "User",
        back_populates="chuyen_dis"
    )

    phuong_tien_di = relationship(
        "PhuongTien",
        foreign_keys=[ma_pt_di]
    )

    phuong_tien_ve = relationship(
        "PhuongTien",
        foreign_keys=[ma_pt_ve]
    )

    tinh_di = relationship(
        "Tinh",
        foreign_keys=[ma_tinh_di]
    )

    tinh_den = relationship(
        "Tinh",
        foreign_keys=[ma_tinh_den]
    )
    ...
    so_thichs = relationship(
        "SoThichCD",
        back_populates="chuyen_di",
        cascade="all, delete"
    )

    loai_du_lichs = relationship(
        "LoaiDuLichCD",
        back_populates="chuyen_di",
        cascade="all, delete"
    )

    yeu_caus = relationship(
        "YeuCauCD",
        back_populates="chuyen_di",
        cascade="all, delete"
    )